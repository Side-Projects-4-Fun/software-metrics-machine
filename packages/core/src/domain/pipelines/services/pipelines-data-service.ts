...
export class PipelinesDataService implements IPipelinesService {
  private tz: TimeZoneProvider;

  constructor(
    private runs: PipelineRun[],
    private deploymentTargets: DeploymentFrequencyTarget[],
  ...
  async getDeploymentFrequencyWithAllIntervals(
    ...
    const targets = this.deploymentTargets;
    ...
    const baseRuns = filters ? this.filterRuns(filters) : this.runs;
    ...
    for (const target of targets) {
      ...
      const deployments = baseRuns.filter(
        (run) =>
  ...
  private filterRuns(filters?: PipelineFilters): PipelineRun[] {
    if (!filters) {
      return this.runs;
    }

    const selectedStatuses = this.parseCsvList(filters.status);
    const selectedConclusions = this.parseCsvList(filters.conclusion);
    const selectedBranches = this.parseCsvList(filters.targetBranch);
    const selectedEvents = this.parseCsvList(filters.event);
    const start = filters.startDate ? this.toDateBoundaryTimestamp(filters.startDate, 'start') : 0;
    const end = filters.endDate ? this.toDateBoundaryTimestamp(filters.endDate, 'end') : 0;
    ...
    if (filters.jobName || filters.jobConclusion) {
      const selectedJobNames = filters.jobName ? this.parseCsvList(filters.jobName) : [];
      const targetJobConclusion = filters.jobConclusion?.trim().toLowerCase();

      filteredRuns = filteredRuns.filter((run) =>
        (run.jobs || []).some((job) => {
          if (selectedJobNames.length > 0 && !selectedJobNames.includes(job.name)) {
            return false;
          }
          if (targetJobConclusion && job.conclusion?.toLowerCase() !== targetJobConclusion) {
            return false;
          }
          return true;
        })
