...
export function mapChangeRequestToDetails(
  pr: PullRequestJsonResponse,
  commentsForChangeRequest: PullRequestCommentJsonResponse[]
  ...
  return {
    ...
    state: pr.state as ChangeRequestDetails['state'],
    url: pr.html_url || '',
    totalComments: commentsForChangeRequest.length,
    comments: commentsForChangeRequest.map((comment) => ({
      url: comment.url,
      body: comment.body,
      change_request_review_id: comment.pull_request_review_id || 0,
      id: comment.id,
...
export function normalizeChangeRequestFilterList(value?: string | string[]): string[] {
  if (!value) {
    return [];
  }
...
export function applyPayloadChangeRequestFilters(
  changeRequests: PullRequestJsonResponse[],
  filters: ChangeRequestFilters | undefined,
  ...
  const labels = normalizeChangeRequestFilterList(filters.labels);
  ...
  return changeRequests.filter((changeRequest) => {
    if (
      !shouldIncludeTimestampForWeekendsMode(
        getChangeRequestMetricDate(changeRequest),
        filters.cleaning?.weekends,
        (dateString) => isChangeRequestWeekday(dateString, timeZoneProvider)
      )
    ) {
      return false;
    }
    ...
    if (
      labelSet &&
      !(changeRequest.labels || []).some((label) => labelSet.has(label.name.toLowerCase()))
    ) {
      return false;
    }
...
export function getChangeRequestMetricDate(changeRequest: PullRequestJsonResponse): string {
  return changeRequest.merged_at || changeRequest.closed_at || changeRequest.created_at;
}

function isChangeRequestWeekday(
