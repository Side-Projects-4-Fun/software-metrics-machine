...
export function parseDashboardFilters(
  ...
  return {
    ...
    ignorePatternFiles:
      getSingleValue(searchParams.ignorePatternFiles) || fallback.ignorePatternFiles,
    includePatternFiles:
      getSingleValue(searchParams.includePatternFiles) || fallback.includePatternFiles,
    authorSelectSourceCode: getArrayValue(searchParams.authorSelectSourceCode),
    topEntries:
      toNumber(getSingleValue(searchParams.topEntries), fallback.topEntries) || fallback.topEntries,
    ...
    sonarqubeRemoveFolders: toBoolean(
      searchParams.sonarqubeRemoveFolders,
      fallback.sonarqubeRemoveFolders
    ),
  };
}
...
function parseWeekends(
  ...
  return value === 'exclude' || value === 'include' || value === 'weekends_only' ? value : fallback;
}
