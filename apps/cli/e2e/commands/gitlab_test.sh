...
function seed_gitlab_mrs_workspace() {
  local workspace="$1"
  seed_sqlite_gitlab_prs_fixture "${workspace}"
}
...
function test_gitlab_prs_help_renders_successfully() {
  run_smm prs --help
  assert_smm_success
...
function test_gitlab_prs_fetch_persists_merge_requests_from_gitlab_api() {
  local workspace
  ...
  run_smm_with_gitlab_api prs fetch \
    --force \
...
function test_gitlab_prs_fetch_comments_persists_comments_from_gitlab_api() {
  local workspace
  ...
  run_smm_with_gitlab_api prs fetch --force --start-date 2026-01-01 --end-date 2026-01-31
  run_smm_with_gitlab_api prs fetch-comments \
    --force \
    --start-date 2026-01-01 \
    --end-date 2026-01-31
...
function test_gitlab_mrs_summary_renders_statistics_from_cached_merge_requests() {
  run_seeded_gitlab_mrs_command prs summary \
    --start-date 2026-01-01 \
    --end-date 2026-01-31 \
    --output json
...
function test_gitlab_mrs_by_month_renders_cached_metrics() {
  run_seeded_gitlab_mrs_command prs by-month \
    --start-date 2026-01-01 \
    --end-date 2026-01-31 \
    --output json
...
function test_gitlab_mrs_by_author_renders_cached_authors() {
  run_seeded_gitlab_mrs_command prs by-author \
    --start-date 2026-01-01 \
    --end-date 2026-01-31 \
    --top 10
...
function test_gitlab_mrs_review_time_renders_with_average_method() {
  run_seeded_gitlab_mrs_command prs review-time \
    --start-date 2026-01-01 \
    --end-date 2026-01-31 \
    --method average \
    --top 10
