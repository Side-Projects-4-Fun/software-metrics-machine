...
interface ChangeRequestEvaluationData {
  generatedAt: string;
...
export default function ChangeRequestEvaluationCard({
  data,
  method,
}: {
  data: ChangeRequestEvaluationData;
  method?: string;
}) {
  const { signals, summary } = data;
  const openTimeLabel = formatMetricLabel(method, 'Open Time');
  const commentsChangeRequestLabel = formatMetricLabel(method, 'Comments/Change Request');
  ...
  const mergeRate = summary.totalChangeRequests > 0
    ? (summary.mergedChangeRequests / summary.totalChangeRequests) * 100
    : 0;
    ...
    <div className="space-y-4">
      <Card>
        <CardHeader className="pb-2">
          <CardTitle>Change Request Health Summary</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="bg-blue-50 rounded-lg p-3 text-center">
              <p className="text-xs text-gray-500">Total Change Requests</p>
              <p className="text-xl font-bold text-blue-700">
                {summary.totalChangeRequests.toLocaleString('en-US')}
              </p>
            </div>
